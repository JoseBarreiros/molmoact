#!/usr/bin/env python3
"""
Interactive EgoDex download script with real-time progress display.
"""
...
def download_with_progress(url: str, output_file: Path, max_retries: int = 3):
    """
    ...
    for attempt in range(max_retries):
        ...
        # Build wget command with progress display
        cmd = [
            'wget',
            ...
            '--progress', 'dot',  # Use dot progress instead of bar
        ...
        try:
            ...
            print("   Downloading... (this may take a while for large files)")
...
def download_egodex_test_interactive(output_dir: str, max_episodes: int = 10):
    """
    Download EgoDex test data with interactive progress.
    """
    output_path = Path(output_dir)
    output_path.mkdir(parents=True, exist_ok=True)
    
    print(f"🚀 Downloading EgoDex test data (interactive version)...")
    print(f"📁 Output directory: {output_path}")
    print(f"📊 Max episodes per task: {max_episodes}")
    print(f"⚠️  Note: The test set is ~16GB, this may take a while...")
    
    # Ask user if they want to continue
    response = input("\n🤔 Do you want to continue with the download? (y/N): ").strip().lower()
    ...
    success = download_with_progress(test_url, test_zip, max_retries=3)
...
def main():
    parser = argparse.ArgumentParser(description="Download EgoDex test data (interactive version)")
    ...
    success = download_egodex_test_interactive(args.output_dir, args.max_episodes)
