...
# EgoDex Scaling Laws Training: 100% EgoDex + 10% MolmoAct
...
export EGODEX_DATA_PERCENTAGE=1.0  # Use 100% of EgoDex data
...
TIMESTAMP=$(date +%Y%m%d_%H%M%S)_100pct

echo "🚀 Starting EgoDex Scaling Laws Training (100% EgoDex)"
echo "📊 EgoDex percentage: 100% of available data"
echo "📊 MolmoAct percentage: 10% (fixed)"
echo "🆔 Run ID: scaling_100pct_${TIMESTAMP}"
...
torchrun --nnodes=1 --nproc-per-node=1 \
    ...
    --wandb.name=egodex_scaling_100pct_${TIMESTAMP} \
    --wandb.entity=your_wandb_entity \
    --wandb.project=molmoact_egodex_scaling_laws \
    --save_folder=checkpoints/egodex_scaling_100pct_${TIMESTAMP} \
    --save_overwrite \
    --duration 50000 \
    --ft_embedding all \
    --depth_tokens \
