...
# EgoDex Scaling Laws Training: 50% EgoDex + 10% MolmoAct
...
export EGODEX_DATA_PERCENTAGE=0.5  # Use 50% of EgoDex data
...
TIMESTAMP=$(date +%Y%m%d_%H%M%S)_50pct

echo "🚀 Starting EgoDex Scaling Laws Training (50% EgoDex)"
echo "📊 EgoDex percentage: 50% of available data"
echo "📊 MolmoAct percentage: 10% (fixed)"
echo "🆔 Run ID: scaling_50pct_${TIMESTAMP}"
...
torchrun --nnodes=1 --nproc-per-node=1 \
    ...
    --wandb.name=egodex_scaling_50pct_${TIMESTAMP} \
    --wandb.entity=your_wandb_entity \
    --wandb.project=molmoact_egodex_scaling_laws \
    --save_folder=checkpoints/egodex_scaling_50pct_${TIMESTAMP} \
    --save_overwrite \
    --duration 50000 \
    --ft_embedding all \
    --depth_tokens \
